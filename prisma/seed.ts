...
import { seedChecklistTemplates } from './seeds/checklist-templates';
...
import { seedVehicleCatalog } from './seeds/vehicle-catalog';
...
export async function seedGlobalKB(prismaParam: PrismaClient): Promise<void> {
  ...
  const temparioKB: { cat: string; name: string }[] = [
    ...
    // Frenos — Sistema neumático (camiones pesados)
    { cat: 'Frenos', name: 'Drenar tanques de aire' },
    { cat: 'Frenos', name: 'Revision frenos de aire' },
    { cat: 'Frenos', name: 'Inspeccion valvula alivio freno' },
    { cat: 'Frenos', name: 'Inspeccion tambores discos freno' },
    // Transmision — diferencial
    { cat: 'Transmision', name: 'Inspeccion diferencial' },
    // Motor — turbo / inyección / refrigeración / secador
    { cat: 'Motor', name: 'Filtro secador de aire' },
    { cat: 'Motor', name: 'Inspeccion turbocharger' },
    { cat: 'Motor', name: 'Inspeccion bomba inyectores' },
    { cat: 'Motor', name: 'Inspeccion radiador refrigeracion' },
    { cat: 'Motor', name: 'Inspeccion mangueras refrigerante' },
    { cat: 'Motor', name: 'Drenaje agua separador combustible' },
  ];
  ...
  // Seeding de Documentos movido a document-templates.ts
  ...
  // await seedHino300Dutro(prisma); // Deprecado: migrado a formato de templates por vehicle type
  // await seedInternational7400WorkStar(prisma); // Deprecado: migrado a formato de templates por vehicle type

  // Catálogo global de marcas y líneas (RUNT Colombia)
  await seedVehicleCatalog(prisma);

  // HSEQ: VehicleTypes globales RUNT + ChecklistTemplates por tipo
  await seedChecklistTemplates(prisma);

  // HSEQ: DocumentTypeConfigs y DocumentRequirements por tipo RUNT
  await seedDocumentTemplates(prisma);

  // Mantenimiento: templates globales base por tipo de vehículo RUNT
  await seedMaintenanceTemplates(prisma);
}
