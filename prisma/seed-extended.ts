...
async function main() {
    ...
    const prado = await prisma.vehicleLine.upsert({
        where: { tenantId_brandId_name: { tenantId: defaultTenant.id, brandId: toyota.id, name: 'Prado' } },
        update: {},
    ...
    const colorado = await prisma.vehicleLine.upsert({
        where: { tenantId_brandId_name: { tenantId: defaultTenant.id, brandId: chevrolet.id, name: 'Colorado' } },
        update: {},
        create: { tenantId: defaultTenant.id, brandId: chevrolet.id, name: 'Colorado' }
    })
    ...
    const trax = await prisma.vehicleLine.upsert({
        where: { tenantId_brandId_name: { tenantId: defaultTenant.id, brandId: chevrolet.id, name: 'Trax' } },
        update: {},
    ...
    const f150 = await prisma.vehicleLine.upsert({
        where: { tenantId_brandId_name: { tenantId: defaultTenant.id, brandId: ford.id, name: 'F-150' } },
        update: {},
    ...
    const explorer = await prisma.vehicleLine.upsert({
        where: { tenantId_brandId_name: { tenantId: defaultTenant.id, brandId: ford.id, name: 'Explorer' } },
        update: {},
    ...
    const frontier = await prisma.vehicleLine.upsert({
        where: { tenantId_brandId_name: { tenantId: defaultTenant.id, brandId: nissan.id, name: 'Frontier' } },
        update: {},
        create: { tenantId: defaultTenant.id, brandId: nissan.id, name: 'Frontier' }
    })

    const sentra = await prisma.vehicleLine.upsert({
        where: { tenantId_brandId_name: { tenantId: defaultTenant.id, brandId: nissan.id, name: 'Sentra' } },
        update: {},
        create: { tenantId: defaultTenant.id, brandId: nissan.id, name: 'Sentra' }
    })

    const xtrail = await prisma.vehicleLine.upsert({
        where: { tenantId_brandId_name: { tenantId: defaultTenant.id, brandId: nissan.id, name: 'X-Trail' } },
        update: {},
    ...
    const quattro = await prisma.vehicleLine.upsert({
        where: { tenantId_brandId_name: { tenantId: defaultTenant.id, brandId: audi.id, name: 'Quattro' } },
        update: {},
    ...
    const c40 = await prisma.vehicleLine.upsert({
        where: { tenantId_brandId_name: { tenantId: defaultTenant.id, brandId: volvo.id, name: 'C40' } },
        update: {},
        create: { tenantId: defaultTenant.id, brandId: volvo.id, name: 'C40' }
    })

    const xc60 = await prisma.vehicleLine.upsert({
        where: { tenantId_brandId_name: { tenantId: defaultTenant.id, brandId: volvo.id, name: 'XC60' } },
        update: {},
    ...
    const rich6 = await prisma.vehicleLine.upsert({
        where: { tenantId_brandId_name: { tenantId: defaultTenant.id, brandId: dongfeng.id, name: 'Rich6' } },
        update: {},
    ...
    const oroch = await prisma.vehicleLine.upsert({
        where: { tenantId_brandId_name: { tenantId: defaultTenant.id, brandId: renault.id, name: 'Oroch' } },
        update: {},
    ...
    const suspensionCategory = await prisma.mantCategory.upsert({
        where: { tenantId_name: { tenantId: defaultTenant.id, name: 'Suspensión' } },
        update: {},
        create: { tenantId: defaultTenant.id, name: 'Suspensión', description: 'Sistema de suspensión' }
    })

    const neumaticosCategory = await prisma.mantCategory.upsert({
        where: { tenantId_name: { tenantId: defaultTenant.id, name: 'Neumáticos' } },
        update: {},
        create: { tenantId: defaultTenant.id, name: 'Neumáticos', description: 'Neumáticos y rines' }
    })

    const fluidosCategory = await prisma.mantCategory.upsert({
        where: { tenantId_name: { tenantId: defaultTenant.id, name: 'Fluidos' } },
        update: {},
        create: { tenantId: defaultTenant.id, name: 'Fluidos', description: 'Aceites y líquidos' }
    })
    ...
    const bujias = await prisma.mantItem.upsert({
        where: { tenantId_name: { tenantId: defaultTenant.id, name: 'Cambio de Bujías' } },
        update: {},
        create: {
            tenantId: defaultTenant.id,
    ...
    const purgadoFreno = await prisma.mantItem.upsert({
        where: { tenantId_name: { tenantId: defaultTenant.id, name: 'Purgado de Frenos' } },
        update: {},
        create: {
            tenantId: defaultTenant.id,
    ...
    const filtroCombustible = await prisma.mantItem.upsert({
        where: { tenantId_name: { tenantId: defaultTenant.id, name: 'Cambio Filtro de Combustible' } },
        update: {},
        create: {
            tenantId: defaultTenant.id,
    ...
    const amortiguadores = await prisma.mantItem.upsert({
        where: { tenantId_name: { tenantId: defaultTenant.id, name: 'Cambio de Amortiguadores' } },
        update: {},
        create: {
            tenantId: defaultTenant.id,
    ...
    const alineacion = await prisma.mantItem.upsert({
        where: { tenantId_name: { tenantId: defaultTenant.id, name: 'Alineación y Balanceo' } },
        update: {},
        create: {
            tenantId: defaultTenant.id,
    ...
    const rotacionNeumaticos = await prisma.mantItem.upsert({
        where: { tenantId_name: { tenantId: defaultTenant.id, name: 'Rotación de Neumáticos' } },
        update: {},
        create: {
            tenantId: defaultTenant.id,
    ...
    await prisma.mantItem.upsert({
        where: { tenantId_name: { tenantId: defaultTenant.id, name: 'Cambio de Neumáticos' } },
        update: {},
        create: {
            tenantId: defaultTenant.id,
    ...
    const refrigerante = await prisma.mantItem.upsert({
        where: { tenantId_name: { tenantId: defaultTenant.id, name: 'Cambio Líquido Refrigerante' } },
        update: {},
        create: {
            tenantId: defaultTenant.id,
    ...
    const tecnico3 = await prisma.technician.upsert({
        where: { tenantId_name: { tenantId: defaultTenant.id, name: 'Luis Martínez' } },
        update: {},
        create: {
            tenantId: defaultTenant.id,
            name: 'Luis Martínez',
    ...
    const proveedor1 = await prisma.provider.upsert({
        where: { tenantId_name: { tenantId: defaultTenant.id, name: 'Taller Central' } },
        update: {},
        create: {
            tenantId: defaultTenant.id,
            name: 'Taller Central',
    ...
    const proveedor2 = await prisma.provider.upsert({
        where: { tenantId_name: { tenantId: defaultTenant.id, name: 'Frenos Especialistas' } },
        update: {},
        create: {
            tenantId: defaultTenant.id,
    ...
    const vehicleData = [
        ...
        { licensePlate: 'JKL-012', brand: nissan, line: frontier, type: pickupType, year: 2020, color: 'Rojo', mileage: 45000, photo: '/images/nissan_frontier_pagina_web.jpg' },
        { licensePlate: 'MNO-345', brand: chevrolet, line: colorado, type: pickupType, year: 2022, color: 'Negro', mileage: 18000, photo: '/images/chevrolet_colorado.jpg' },
        { licensePlate: 'PQR-678', brand: dongfeng, line: rich6, type: pickupType, year: 2023, color: 'Plata', mileage: 5000, photo: '/images/dongfeng_rich6.jpg' },
        { licensePlate: 'STU-901', brand: renault, line: oroch, type: pickupType, year: 2021, color: 'Blanco', mileage: 32000, photo: '/images/oroch.jpg' },
        { licensePlate: 'DAS-144', brand: ford, line: f150, type: pickupType, year: 2022, color: 'Azul', mileage: 19000, photo: '/images/DAS-144.jpg' },
        { licensePlate: 'DAS-157', brand: chevrolet, line: dmax, type: pickupType, year: 2021, color: 'Blanco', mileage: 41000, photo: '/images/DAS-157.jpg' },
        { licensePlate: 'DAR-974', brand: toyota, line: hilux, type: pickupType, year: 2020, color: 'Rojo', mileage: 52000, photo: '/images/DAR-974.jpg' },

        // AUTOMÓVILES
        { licensePlate: 'VWX-234', brand: audi, line: quattro, type: autoType, year: 2023, color: 'Negro', mileage: 12000, photo: '/images/audi_quattro.jpeg' },
        { licensePlate: 'YZA-567', brand: volvo, line: c40, type: autoType, year: 2024, color: 'Blanco', mileage: 3000, photo: '/images/volvo_c40_white.jpg' },
        ...
        { licensePlate: 'ZAB-234', brand: nissan, line: sentra, type: autoType, year: 2023, color: 'Blanco', mileage: 6500, photo: '/images/nissan_frontier_pagina_web.jpg' },

        // CAMIONETAS
        { licensePlate: 'BCD-890', brand: toyota, line: prado, type: camionetaType, year: 2022, color: 'Gris Oscuro', mileage: 28000, photo: '/images/hilux.jpg' },
        { licensePlate: 'EFG-123', brand: chevrolet, line: trax, type: camionetaType, year: 2023, color: 'Rojo', mileage: 9000, photo: '/images/chevrolet_cyz.jpg' },
        { licensePlate: 'HIJ-456', brand: ford, line: explorer, type: camionetaType, year: 2021, color: 'Azul Marino', mileage: 35000, photo: '/images/ford_ranger_pagina_web.jpg' },
        { licensePlate: 'KLM-789', brand: nissan, line: xtrail, type: camionetaType, year: 2022, color: 'Blanco Perla', mileage: 22000, photo: '/images/nissan_frontier_pagina_web_1.jpg' },
        { licensePlate: 'DAS-141', brand: volvo, line: xc60, type: camionetaType, year: 2023, color: 'Negro', mileage: 11000, photo: '/images/DAS-141.jpg' }
    ]
    ...
    for (const planData of plans) {
        ...
        for (const taskData of planData.items) {
            await prisma.planTask.upsert({
                where: { planId_mantItemId: { planId: plan.id, mantItemId: taskData.item.id } },
                update: {},
                create: {
                    planId: plan.id,
    ...
    for (const vehicle of vehicles) {
        ...
        const compatiblePlan = createdPlans.find(plan => 
                ...
                for (let cycle = cycles; cycle <= cycles + 3; cycle++) {
                    ...
                    if (executionMileage <= currentKm) {
                        ...
                        if (random < 0.7) {
                            ...
                            endDate = new Date(startDate.getTime() + Math.random() * 8 * 60 * 60 * 1000)
                    ...
                    try {
                        await prisma.vehicleMantPlanItem.create({
                            data: {
                                ...
                                executionMileage: executionMileage + Math.floor(Math.random() * 50), // Pequeña variación
                                status,
                                startDate,
                                endDate,
                                technicianId,
    ...
    console.log('   - Cientos de items con diferentes estados (PENDING, IN_PROGRESS, COMPLETED)')
