...
export class WhatsAppService {
  ...
  constructor() {
    ...
    if (!accountSid || !authToken || !phoneNumber) {
      throw new Error(
        'Missing Twilio configuration. Check TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, and TWILIO_PHONE_NUMBER in environment variables.'
      );
    }
  ...
  async sendMessage(message: WhatsAppMessage): Promise<WhatsAppResponse> {
    try {
      ...
      console.log(
        `✅ WhatsApp sent to ${formattedPhone}: ${twilioMessage.sid}`
      );
  ...
  async sendBatchMessages(
    ...
    console.log(
      `📊 Batch WhatsApp Summary: ${response.totalSent} sent, ${response.totalFailed} failed`
    );
  ...
  async validateConfiguration(): Promise<{ valid: boolean; error?: string }> {
    try {
      ...
      console.log(
        `✅ Twilio configuration valid. Account: ${account.friendlyName}`
      );
