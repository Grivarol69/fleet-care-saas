...
describe('MaintenanceAlertService', () => {
    ...
    describe('checkAndGenerateAlerts', () => {
        it('should generate an EARLY_WARNING alert when maintenance is approaching', async () => {
            ...
            (prisma.vehicleMantProgram.findFirst as any).mockResolvedValue(mockProgram);
            (prisma.maintenanceAlert.findFirst as any).mockResolvedValue(null); // No previous alert

            // 2. Act
            await MaintenanceAlertService.checkAndGenerateAlerts(mockVehicleId, currentKm, mockTenantId);

            // 3. Assert
            expect(prisma.maintenanceAlert.create).toHaveBeenCalledTimes(1);
            const createCall = (prisma.maintenanceAlert.create as any).mock.calls[0][0];
        ...
        it('should update existing alert instead of creating duplicate', async () => {
            ...
            (prisma.vehicleMantProgram.findFirst as any).mockResolvedValue(mockProgram);
            (prisma.maintenanceAlert.findFirst as any).mockResolvedValue(mockExistingAlert);
            ...
            const updateCall = (prisma.maintenanceAlert.update as any).mock.calls[0][0];
