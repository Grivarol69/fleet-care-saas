    ...
    query: {
      $allModels: {
        async $allOperations({ model, operation, args, query }) {
          ...
          // findUnique/findUniqueOrThrow: no pueden usar OR en where (rompe Prisma)
          // Para modelos globales: no filtrar por tenantId (recursos compartidos)
          // Para modelos normales: inyectar tenantId para aislamiento
          ...
          // 3. Operaciones que requieren restricciones Unique (update, delete, upsert)
          ...
          if (operation === 'upsert') {
            ...
            argsClone.create = argsClone.create
              ? { ...(argsClone.create as any), tenantId }
              : { tenantId };
