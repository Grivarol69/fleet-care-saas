...
export function FormEditPurchase({
  ...
  type FormValues = z.infer<typeof formEditPurchaseSchema>;
  const form = useForm<FormValues, unknown, FormValues>({
    ...
    defaultValues: {
      ...
      supplierId:
        (purchase.supplier as { id: string })?.id ||
        (purchase as { supplierId?: string }).supplierId ||
        '',
    },
  });
  ...
  const onSubmit = async (values: z.infer<typeof formEditPurchaseSchema>) => {
    try {
      ...
      const updatedPurchase = {
        ...
        supplier:
          providers.find(p => p.id === response.data.supplierId) ||
          purchase.supplier,
      };

      onEditPurchase(updatedPurchase as PurchaseInvoiceRow);
      setIsOpen(false);
      form.reset();
