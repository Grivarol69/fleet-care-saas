...
import { FormEditTemplateItemProps, MantItem } from './FormEditTemplateItem.types';
...
export function FormEditTemplateItem({
  isOpen,
  setIsOpen,
  item,
  onEditItem,
}: FormEditTemplateItemProps) {
  ...
  useEffect(() => {
    const fetchMantItems = async () => {
      try {
        ...
        setMantItems(response.data.filter((mantItem: MantItem) => mantItem.status === 'ACTIVE'));
      } catch (error) {
  ...
  const onSubmit = async (values: FormData) => {
    try {
      ...
      const response = await axios.patch(`/api/maintenance/template-items/${item.id}`, payload);
      ...
      toast({
        ...
        description: `La tarea "${updatedItem.mantItem.name}" fue actualizada exitosamente.`,
      });
      ...
      let errorMessage = 'Algo salió mal al actualizar la tarea';
      if (axios.isAxiosError(error)) {
        if (error.response?.status === 409) {
          ...
          errorMessage = 'No autorizado para actualizar tareas';
        } else if (error.response?.status === 404) {
          errorMessage = 'Tarea o item de mantenimiento no encontrado';
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Editar Tarea del Template</DialogTitle>
        </DialogHeader>
        <Form {...form}>
                  ...
                  >
                    ...
                    <SelectContent>
                      {mantItems.length === 0 && !isLoadingItems ? (
                        ...
                        mantItems.map((mantItem: MantItem) => (
                          <SelectItem
                            key={mantItem.id}
                            value={mantItem.id.toString()}
                          >
                            <div className="flex flex-col">
                              <span className="font-medium">{mantItem.name}</span>
                              <span className="text-sm text-gray-500">
                                {formatMantType(mantItem.mantType)} • {mantItem.category.name} • {Number(mantItem.estimatedTime).toFixed(1)}h
                              </span>
                            </div>
                          </SelectItem>
                        ))
