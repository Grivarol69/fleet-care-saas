...
export function FormAddVehiclePart({
    ...
    const onSubmit = async (values: FormValues) => {
        try {
            setIsLoading(true);
            const response = await axios.post('/api/maintenance/vehicle-parts', values);
            onAdd(response.data);
            setIsOpen(false);
            form.reset();
            ...
            if (axios.isAxiosError(error)) {
                ...
                if (error.response?.status === 403) {
                    toast({
                        title: 'Sin permisos',
                        description: error.response.data?.error ?? 'No tiene permisos para esta accion',
                        variant: 'destructive',
                    });
                    return;
                }
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    >
                                        <FormControl>
                                            <SelectTrigger>
                                                <SelectValue placeholder={selectedBrandId ? 'Seleccionar linea...' : 'Seleccione marca primero'} />
                                            </SelectTrigger>
                                        </FormControl>
                                        <SelectContent>
                                            {filteredLines.map((l) => (
