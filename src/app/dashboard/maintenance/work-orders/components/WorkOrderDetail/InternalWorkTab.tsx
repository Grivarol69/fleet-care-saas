...
export function InternalWorkTab({ workOrderId, onRefresh }: Props) {
    ...
    useEffect(() => {
        ...
        fetchTickets();
        fetchTechnicians();
    }, [fetchItems, fetchTickets, fetchTechnicians]);
    ...
    if (isLoadingItems || isLoadingTickets) {
        return (
    ...
    const sumTotalLaborHours = tickets.reduce((acc, t) => acc + Number(t.totalLaborHours || 0), 0);
    const sumTotalLaborCost = tickets.reduce((acc, t) => acc + Number(t.totalLaborCost || 0), 0);
    const sumTotalPartsCost = tickets.reduce((acc, t) => acc + Number(t.totalPartsCost || 0), 0);
    const sumGrandTotal = tickets.reduce((acc, t) => acc + Number(t.totalCost || 0), 0);
                            ...
                            <Card key={ticket.id} className="overflow-hidden">
                                <div className="bg-muted p-3 flex justify-between items-center bg-gray-50 border-b">
                                    <div className="flex items-center gap-3">
                                        <span className="font-semibold">Ticket #{ticket.ticketNumber || ticket.id.slice(-6).toUpperCase()}</span>
                                ...
                                <div className="p-4 space-y-4">
                                            ...
                                            <Table>
                                                <TableHeader>
                                                    <TableRow className="border-b">
                                                        ...
                                                        <TableHead className="h-8 py-1 text-right">Horas</TableHead>
                                                        <TableHead className="h-8 py-1 text-right">Tarifa/hr</TableHead>
                                                ...
                                                <TableBody>
                                                    {ticket.laborEntries.map((l, idx) => (
                                                        <TableRow key={idx}>
                                                            <TableCell className="py-2">{l.description}</TableCell>
                                                            <TableCell className="py-2 text-right">{Number(l.hours).toFixed(1)}</TableCell>
                                                            <TableCell className="py-2 text-right">{formatCurrency(Number(l.hourlyRate))}</TableCell>
                                                            <TableCell className="py-2 text-right font-medium">{formatCurrency(Number(l.laborCost))}</TableCell>
                                                        </TableRow>
                                                    ))}
                                                </TableBody>
                                                <tfoot>
                                                    <TableRow>
                                                        <TableCell className="font-medium py-2">Total Mano de Obra</TableCell>
                                                        <TableCell className="text-right font-bold py-2">{Number(ticket.totalLaborHours || 0).toFixed(1)}</TableCell>
                                                        <TableCell />
                                                        <TableCell className="text-right font-bold py-2">{formatCurrency(Number(ticket.totalLaborCost))}</TableCell>
                                                    </TableRow>
                                                </tfoot>
                                            </Table>
                                        </div>
                                            ...
                                            <Table>
                                                <TableHeader>
                                                    <TableRow className="border-b">
                                                        ...
                                                        <TableHead className="h-8 py-1 text-right">Costo Unit</TableHead>
                                                        <TableHead className="h-8 py-1 text-right">Total</TableHead>
                                                    </TableRow>
                                                </TableHeader>
                                                <TableBody>
                                                    {ticket.partEntries.map((p, idx) => (
                                                        <TableRow key={idx}>
                                                            <TableCell className="py-2">{p.inventoryItem.masterPart?.code || '-'}</TableCell>
                                                            <TableCell className="py-2">{p.inventoryItem.masterPart?.description || '-'}</TableCell>
                                                            <TableCell className="py-2 text-right">{Number(p.quantity)}</TableCell>
                                                            <TableCell className="py-2 text-right">{formatCurrency(Number(p.unitCost))}</TableCell>
                                                            <TableCell className="py-2 text-right font-medium">{formatCurrency(Number(p.totalCost))}</TableCell>
                                                        </TableRow>
                                                    ))}
                                                </TableBody>
                                                <tfoot>
                                                    <TableRow>
                                                        <TableCell colSpan={4} className="font-medium text-right py-2">Total Repuestos</TableCell>
                                                        <TableCell className="text-right font-bold py-2">{formatCurrency(Number(ticket.totalPartsCost))}</TableCell>
                                                    </TableRow>
                                                </tfoot>
                                            </Table>
                                        </div>
                                    ...
                                    <div className="flex justify-end pt-2 border-t text-sm">
                                        <span className="text-muted-foreground mr-4">Total Ticket:</span>
                                        <span className="font-bold text-lg">{formatCurrency(Number(ticket.totalCost))}</span>
                                    </div>
                                </div>
                            </Card>
                        ))
                    ...
                    <CardContent className="p-6">
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                            <div className="space-y-1">
                                <span className="text-sm text-slate-500 font-medium uppercase tracking-wider">Total Horas</span>
                                <p className="text-2xl font-bold">{sumTotalLaborHours.toFixed(1)} hs</p>
                            </div>
                            <div className="space-y-1">
                                <span className="text-sm text-slate-500 font-medium uppercase tracking-wider">Total Mano de Obra</span>
                                <p className="text-2xl font-bold text-slate-700">{formatCurrency(sumTotalLaborCost)}</p>
                            </div>
                            <div className="space-y-1">
                                <span className="text-sm text-slate-500 font-medium uppercase tracking-wider">Total Repuestos</span>
                                <p className="text-2xl font-bold text-slate-700">{formatCurrency(sumTotalPartsCost)}</p>
                            </div>
                            <div className="space-y-1 border-l pl-6 border-slate-200">
                                <span className="text-sm text-slate-500 font-medium uppercase tracking-wider">Costo Taller Total</span>
                                <p className="text-3xl font-black text-slate-900">{formatCurrency(sumGrandTotal)}</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
            ...
            <Dialog open={showDialog} onOpenChange={setShowDialog}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Se creará un ticket para {pendingItemsIds.length} tarea(s) de taller pendiente(s).
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="space-y-4 py-4">
                        ...
                        <div className="rounded-md bg-muted p-3">
                            ...
                            <ul className="text-sm space-y-1 list-disc list-inside">
                                {items.filter(i => i.closureType === 'PENDING').map(item => (
