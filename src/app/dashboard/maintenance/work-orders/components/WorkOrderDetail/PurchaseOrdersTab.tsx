...
export function PurchaseOrdersTab({ workOrderId }: PurchaseOrdersTabProps) {
  ...
  const handleAction = (poId: string, action: string) => {
    ...
    let sendDescription = "La orden se enviará por email al proveedor con el PDF adjunto.";
    if (action === "send") {
      ...
      if (po?.provider.email) {
        ...
        toast({
          ...
          description: "El proveedor no tiene email configurado. Actualice los datos del proveedor antes de enviar la OC.",
          variant: "destructive",
        });
        return;
      }
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Las órdenes de compra se crean al asignar items externos a proveedores
            </p>
          </CardContent>
        </Card>
      ) : (
