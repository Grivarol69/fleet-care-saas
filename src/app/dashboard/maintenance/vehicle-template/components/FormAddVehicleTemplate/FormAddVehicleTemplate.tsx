...
import { formSchema } from "./FormAddVehicleTemplate.form";
import {
  FormAddVehicleTemplateProps,
  Vehicle,
  MantPlan,
} from "../types";
...
export function FormAddVehicleTemplate({
  isOpen,
  setIsOpen,
  onAddVehicleTemplate,
}: FormAddVehicleTemplateProps) {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      const response = await axios.post("/api/maintenance/vehicle-template", {
      ...
      onAddVehicleTemplate(response.data);
      setIsOpen(false);
      form.reset();
      ...
      toast({
        ...
        description: "La plantilla de mantenimiento ha sido asignada correctamente",
      });
    } catch (error) {
      console.error("Error creating vehicle template:", error);
      toast({
        title: "Error al crear plantilla",
        description: "No se pudo asignar el plan de mantenimiento al vehículo",
        variant: "destructive",
      });
    } finally {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Asignar Plan de Mantenimiento</DialogTitle>
        </DialogHeader>
