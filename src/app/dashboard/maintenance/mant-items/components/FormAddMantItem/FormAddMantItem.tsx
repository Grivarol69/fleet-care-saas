...
// Tipos corregidos según el schema de Prisma
...
export function FormAddMantItem({
  ...
  const onSubmit = async (values: FormData) => {
    try {
      ...
      console.error('Error creating mant item:', error);

      let errorMessage = 'Algo salió mal al crear el item de mantenimiento';
      if (axios.isAxiosError(error)) {
        if (error.response?.status === 409) {
          ...
          errorMessage = 'No autorizado para crear items de mantenimiento';
