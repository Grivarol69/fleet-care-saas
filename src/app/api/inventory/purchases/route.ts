...
export async function POST(req: Request) {
    try {
        ...
        const taxAmount = body.items.reduce((sum, item) => sum + (item.quantity * item.unitPrice * (item.taxRate / 100)), 0);
            ...
            for (const item of body.items) {
                ...
                await tx.inventoryMovement.create({
                    data: {
                        ...
                        newAvgCost: invItem.averageCost ? Number(invItem.averageCost) : item.unitPrice,
                ...
                const masterPart = await tx.masterPart.findUnique({ where: { id: item.masterPartId } });
                if (masterPart && masterPart.referencePrice) {
                    const refPrice = Number(masterPart.referencePrice);
                    if (item.unitPrice > refPrice * 1.1) { // 10% tolerance
                        await tx.financialAlert.create({
                            data: {
                                ...
                                details: { expected: refPrice, actual: item.unitPrice, deviation: item.unitPrice - refPrice },
