  ...
  try {
    ...
    if (user.role !== 'OWNER') {
      return NextResponse.json(
        {
          error:
            'Solo el propietario puede modificar la configuración del tenant',
        },
        { status: 403 }
      );
    }
  ...
  try {
    ...
    if (tenant._count.users > 1 || tenant._count.vehicles > 0) {
      return NextResponse.json(
        {
          error:
            'No se puede eliminar un tenant con otros usuarios o vehículos. Primero elimine todos los datos.',
        },
        { status: 400 }
      );
    }
