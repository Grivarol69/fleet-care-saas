...
export async function GET(request: NextRequest) {
  ...
  if (!canViewCosts(user)) {
    return NextResponse.json(
      { error: 'Sin permisos para ver análisis de combustible' },
      { status: 403 }
    );
  }
  ...
  const toParam =
    searchParams.get('to') ?? new Date().toISOString().slice(0, 7);

  if (!vehicleId) {
    return NextResponse.json(
      { error: 'Falta parámetro vehicleId' },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    console.error('[ANALYTICS_FUEL] GET error:', error);
    return NextResponse.json(
