...
export async function GET(request: NextRequest) {
  ...
  if (!fromRange || !toRange) {
    return NextResponse.json(
      { error: 'Parámetros from y to requeridos (formato YYYY-MM)' },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    const purchasesTotal = invoices.reduce(
    ...
    if (format === 'pdf') {
      ...
      const buffer = await renderToBuffer(
        React.createElement(VehicleCostsPDF, result) as any
      );
    ...
    console.error('[MAINTENANCE_COSTS] GET error:', error);
    return NextResponse.json(
