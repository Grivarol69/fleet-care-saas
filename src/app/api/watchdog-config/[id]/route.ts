...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  try {
    const config = await tenantPrisma.watchdogConfiguration.findUnique({
      where: { id },
    });
    ...
    if (typeof threshold !== 'number' || threshold < 1 || threshold > 100) {
      return NextResponse.json(
        { error: 'threshold debe ser un número entre 1 y 100' },
        { status: 400 }
      );
    }
    ...
    console.error('[WATCHDOG_CONFIG] PATCH error:', error);
    return NextResponse.json(
...
export async function DELETE(_request: NextRequest, { params }: Params) {
  ...
  try {
    ...
    console.error('[WATCHDOG_CONFIG] DELETE error:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
