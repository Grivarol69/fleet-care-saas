...
export async function POST(req: NextRequest) {
  try {
    // 1. Verify User Role -> platform SUPER_ADMIN
...
Usa la herramienta proporcionada para estructurar la respuesta JSON.`,
      ...
      tools: [
        {
          ...
          description:
            'Genera el plan de mantenimiento estructurado extraído del manual.',
          input_schema: {
            type: 'object',
            properties: {
              vehicleInfo: {
    ...
    if (!toolCall || toolCall.type !== 'tool_use') {
      return NextResponse.json(
        { error: 'Claude AI did not return a structured plan.' },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json(
      { error: 'Internal Server Error while process AI vision.' },
      { status: 500 }
    );
  }
}
