...
export async function GET(request: NextRequest) {
  try {
    ...
    if (status) {
      ...
      where.status = {
        in: ['PENDING', 'ACKNOWLEDGED', 'SNOOZED', 'IN_PROGRESS'],
      };
    }
    ...
    const formattedAlerts = alerts.map(alert => ({
      ...
      vehiclePhoto:
        alert.vehicle.photo ||
        'https://utfs.io/f/ed8f2e8a-1265-4310-b086-1385aa133fc8-zbbdk9.jpg',
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const { alertId, status, notes, snoozedUntil, snoozeReason, cancelReason } =
      validation.data;
