...
export function VehicleSelector({ vehicles }: { vehicles: VehicleOption[] }) {
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="max-h-[50vh] overflow-y-auto p-2 space-y-2">
          {filteredVehicles.length === 0 ? (
            <div className="p-6 text-center text-gray-500">No se encontraron vehículos.</div>
          ) : (
            filteredVehicles.map(v => (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${selectedId === v.id ? 'bg-[#1E3A5F]' : 'bg-gray-100'}`}>
                    <Truck className={`w-6 h-6 ${selectedId === v.id ? 'text-white' : 'text-gray-500'}`} />
                  </div>
                  <div>
                    <h3 className="text-lg font-bold text-gray-800">{v.licensePlate}</h3>
                    <p className="text-sm text-gray-500">{v.brand?.name} {v.line?.name}</p>
                  </div>
                </div>
                {selectedId === v.id && (
