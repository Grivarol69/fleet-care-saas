...
export async function copyKnowledgeBaseToTenant(
  ...
  console.log(
    '[CopyKB] Starting copy for tenant:',
    tenantId,
    'options:',
    options
  );
  ...
  if (
    !options.vehicleMetadata &&
    !options.maintenanceItems &&
    options.lineIds.length === 0
  ) {
    ...
    const result = await prisma.$transaction(
      async tx => {
        ...
        if (options.maintenanceItems) {
          ...
          console.log(
            '[CopyKB] Copying intelligent links (MantItemVehiclePart)...'
          );
        ...
        if (options.lineIds.length > 0) {
          console.log(
            '[CopyKB] Copying maintenance templates for lines:',
            options.lineIds
          );
          ...
          for (const tpl of globalTemplates) {
            ...
            if (!newLineId) {
              console.log(
                '[CopyKB] Skipping template, line not found:',
                tpl.vehicleLineId
              );
              continue;
            }

            const newBrandId = tpl.vehicleBrandId
              ? brandMap.get(tpl.vehicleBrandId)
              : null;
            if (!newBrandId) {
              console.log(
                '[CopyKB] Skipping template, brand not found:',
                tpl.vehicleBrandId
              );
              continue;
            }
            ...
            for (const pkg of tpl.packages) {
              ...
              for (const pi of pkg.packageItems) {
                ...
                if (!newMantItemId) {
                  console.log(
                    '[CopyKB] Skipping package item, item not found:',
                    pi.mantItemId
                  );
                  continue;
                }
