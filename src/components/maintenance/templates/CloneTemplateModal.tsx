...
export function CloneTemplateModal({ templateId, originalName, onSuccess }: CloneTemplateModalProps) {
    ...
    useEffect(() => {
        if (open && brands.length === 0) {
            const fetchBrands = async () => {
                ...
                try {
                    const res = await fetch("/api/vehicles/settings/brands");
    ...
    useEffect(() => {
        if (selectedBrand) {
            const fetchLines = async () => {
                ...
                try {
                    const res = await fetch(`/api/vehicles/settings/lines?brandId=${selectedBrand}`);
    ...
    const handleClone = async () => {
        ...
        try {
            ...
            toast.info("Importante: Recuerda agregar los repuestos específicos para este nuevo vehículo.", { duration: 6000 });
            setOpen(false);
            if (onSuccess) {
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[425px]">
                ...
                <div className="grid gap-4 py-4">
                    ...
                    <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900 rounded-md p-3 flex items-start gap-3">
                        ...
                        <div className="text-sm text-amber-800 dark:text-amber-400">
                            <strong>Atención:</strong> Para prevenir cruce de datos, el nuevo plan <strong>NO incluirá</strong> los repuestos automotrices del plan original. Deberás vincular repuestos específicos en el CRUD del nuevo plan.
                        </div>
                    </div>
                    ...
                    <div className="grid gap-2">
                        ...
                        <Select onValueChange={setSelectedBrand} value={selectedBrand} disabled={isLoadingBrands}>
                    ...
                    <div className="grid gap-2">
                        ...
                        <Select onValueChange={setSelectedLine} value={selectedLine} disabled={!selectedBrand || isLoadingLines}>
                            <SelectTrigger id="line">
                                <SelectValue placeholder={!selectedBrand ? "Selecciona una marca primero" : isLoadingLines ? "Cargando..." : "Selecciona una línea"} />
                            </SelectTrigger>
                            <SelectContent>
                                {lines.map((l) => (
                        ...
                        {!selectedBrand && <span className="text-xs text-muted-foreground">Si no encuentras la línea, regístrala desde Catálogos primero.</span>}
                    </div>
                ...
                <DialogFooter>
                    ...
                    <Button onClick={handleClone} disabled={isCloning || !name || !selectedBrand || !selectedLine}>
