                            ...
                            <div key={alert.id} className="flex items-start justify-between border-b pb-4 last:border-0 last:pb-0">
                                <div className="space-y-1">
                                    <div className="flex items-center gap-2">
                                        <Badge variant="outline" className={severityColor[alert.severity as keyof typeof severityColor] || "bg-gray-100"}>
                                            {alert.severity}
                                        </Badge>
                                    ...
                                    {(alert.workOrder || alert.masterPart) && (
                                        <div className="flex gap-2 text-xs text-muted-foreground mt-1">
                                            {alert.workOrder && <span>Orden #{alert.workOrder.id}</span>}
                                            {alert.masterPart && <span>Parte: {alert.masterPart.code}</span>}
                                        </div>
                                    )}
                                </div>
                                <Button variant="ghost" size="sm" onClick={() => window.location.href = `/dashboard/maintenance/work-orders/${alert.workOrder?.id}?tab=expenses`}>
                                    Revisar
                                </Button>
                            </div>
                        ))}
