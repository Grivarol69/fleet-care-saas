...
export function QuickOdometerModal({
  ...
  const [measureType, setMeasureType] = useState<"KILOMETERS" | "HOURS">("KILOMETERS");
  ...
  const fetchVehicles = async () => {
    try {
      ...
      const fleetData = response.data.vehicles.map((v: { id: number; licensePlate: string; brandName: string; lineName: string; currentMileage: number; odometer: { lastUpdate: Date | null; daysSinceUpdate: number } }) => ({
        id: v.id,
      ...
      toast({
        ...
        description: (
          <div className="flex flex-col gap-1">
            <span className="font-semibold">{selectedVehicle.licensePlate}</span>
            <span>
              {numValue.toLocaleString()} {measureType === "KILOMETERS" ? "km" : "hrs"}
              {measureType === "KILOMETERS" && increment > 0 && (
                <span className="text-green-600 ml-2">+{increment.toLocaleString()} km</span>
              )}
            </span>
          </div>
        ),
        ...
        <div className="space-y-5 py-4" onKeyDown={handleKeyDown}>
            ...
            <Popover open={vehicleOpen} onOpenChange={setVehicleOpen}>
              <PopoverTrigger asChild>
                ...
                >
                  {selectedVehicle ? (
                    ...
                    <span className="text-muted-foreground">Buscar por placa...</span>
                  )}
              ...
              <PopoverContent className="w-[450px] p-0" align="start">
                <Command>
                  ...
                  <CommandList>
                    <CommandEmpty>
                      {loadingVehicles ? "Cargando..." : "No se encontró el vehículo"}
                    </CommandEmpty>
                    <CommandGroup>
          ...
          <div className="space-y-2">
            <Label className="flex items-center gap-2">
              ...
              Conductor <span className="text-xs text-muted-foreground">(opcional)</span>
            </Label>
          ...
          <div className="grid grid-cols-5 gap-3">
            ...
            <div className="col-span-3 space-y-2">
              <Label>{measureType === "KILOMETERS" ? "Kilometraje" : "Horas"}</Label>
          ...
          {selectedVehicle && value && parseInt(value) > selectedVehicle.mileage && (
            <div className="flex items-center justify-center gap-2 py-3 bg-green-50 rounded-lg border border-green-200">
              ...
              <Badge className="bg-green-600 text-white text-base px-3 py-1">
                +{(parseInt(value) - selectedVehicle.mileage).toLocaleString()} km
              </Badge>
            </div>
          )}
        </div>
